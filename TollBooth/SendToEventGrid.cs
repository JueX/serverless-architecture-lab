    ...
    public class SendToEventGrid
    {
        ...
        public async Task SendLicensePlateData(LicensePlateData data)
        {
            ...
            if (data.LicensePlateFound)
            {
                ...
                await Send("savePlateData", "TollBooth/CustomerService", data);// COMPLETE: await Send(...);
            }
            else
            {
                // TODO 4: Modify send method to include the proper eventType name value for queuing plate for manual review.
                await Send("queuePlateForManualCheckup", "TollBooth/CustomerService", data);// COMPLETE: await Send(...);
            }
        }
